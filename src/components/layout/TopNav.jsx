...
import { Bell, Search, User, Check, X, Menu } from 'lucide-react';
import { useStore } from '../../store/useStore';
import { useSound } from '../../hooks/useSound.js';

export default function TopNav() {
  const { settings, notifications, markNotificationsAsRead, toggleSidebar } = useStore();
  ...
  return (
    <div className="page-padding" style={{ paddingBottom: '0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px', position: 'relative', zIndex: 100 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1, minWidth: 0 }}>
        <button 
          ...
          style={{ width: '48px', height: '48px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', backgroundColor: 'var(--card-bg)', border: '2px solid var(--border-color)', boxShadow: 'var(--shadow-hard-small)', flexShrink: 0 }}
        >
          <Menu size={24} />
        </button>
        
        <div style={{ position: 'relative', width: '100%', maxWidth: '320px', minWidth: '100px' }}>
          <Search size={20} style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: 'var(--gray-400)' }} />
          <input 
            type="text" 
            placeholder="Search..." 
            className="input-base"
            style={{ paddingLeft: '44px', height: '48px', borderRadius: '48px', width: '100%' }}
          />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div style={{ position: 'relative' }} ref={dropdownRef}>
          <button 
            ...
            style={{ width: '48px', height: '48px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', backgroundColor: 'var(--card-bg)', position: 'relative', cursor: 'pointer', flexShrink: 0 }}
          >
            <Bell size={20} />
            {unreadCount > 0 && (
          ...
          {showNotifications && (
            <div className="sticker-card" style={{ 
              position: 'absolute', top: 'calc(100% + 12px)', right: 0, width: '320px', maxWidth: 'calc(100vw - 48px)',
        ...
        <div className="desktop-only" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '4px 8px 4px 16px', borderRadius: '999px', backgroundColor: 'var(--card-bg)', border: '2px solid var(--border-color)', boxShadow: 'var(--shadow-hard-small)' }}>
